...
module Blacklight::Catalog
    ...
    included do
      ...
      def search_action_url options = {}
       ...
       # FIXED: only allow user to email themselves. 
       def email_action documents
        if current_user.present?
         #mail = RecordMailer.email_record(documents, {:to => params[:to], :message => params[:message]}, url_options)
         mail = RecordMailer.email_record(documents, {:to => current_user, :message => params[:message]}, url_options)
...
=end
      ...
      def validate_email_params
        # we're not checking for the 'to' field of the form since it's being sent to the current_user
=begin
        if params[:to].blank?
